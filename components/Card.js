    ...
    <div className="block bg-white rounded-lg shadow-md overflow-hidden relative" style={{ minWidth: '300px', height: 'auto' }}>
      <Link href={`/experience/${id}`}>
        <div className="relative w-full h-40">
          ...
          {/* Slots Left */}
          <div className="absolute bottom-0 left-0 bg-white bg-opacity-50  text-black  py-1 px-3 rounded-t  text-sm">
            <FaUsers className="inline mr-1"/>
            {slotsBooked} Slots Left
          </div>
          {/* Rating */}
          <div className="absolute flex bottom-0 right-0 bg-white bg-opacity-40 text-black  py-1 rounded-t px-3 text-sm  ">
            {ratingStars}
